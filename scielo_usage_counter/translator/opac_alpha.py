...
REGEX_OPAC_ALPHA_ARTICLE_ACRONYM_YEAR_VOL_ISSUE_LANGUAGE = re.compile(r'article/(?P<journal_acronym>\w*)/(?P<year>\d{0,4})\.(?P<vol_issue>[\d|\w]+)/(?P<pages>[\d|-]*)/(?P<language>\w{2})', re.IGNORECASE) # Request or Investigation
REGEX_OPAC_ALPHA_PDF_ACRONYM_YEAR_VOL_ISSUE_LANGUAGE = re.compile(r'pdf/(?P<journal_acronym>\w*)/(?P<year>\d{0,4})\.(?P<vol_issue>[\d|\w]+)/(?P<pages>[\d|-]*)/(?P<language>\w{2})', re.IGNORECASE) # Request
REGEX_OPAC_ALPHA_PDF_PATH = r'.*/?pdf/(?P<journal_acronym>\w*)/(.*)'    # Request
REGEX_OPAC_ALPHA_MEDIA_ASSETS_ACRONYM = re.compile(r'.*/media/assets/(?P<journal_acronym>\w*)/?([\d|\w|\.|\-]*)/?([\d|\w|\-|\.|\_]*)', re.IGNORECASE)   # Request
REGEX_OPAC_ALPHA_RESOURCE_SSM_PATH = r'.*resource_ssm_path=(?P<journal_acronym>\w*)/(?P<year_vol_issue>[\d|\w|\.|\-]*)/(?P<file>[\d|\w|\.|\-]*)'    # Request
...
class URLTranslatorOPACAlphaSite:
    ...
    def _get_url_params_from_query_key(self, url_params, query):
        ...
        match = re.search(REGEX_OPAC_ALPHA_RESOURCE_SSM_PATH, resource_path)
        if match:
            url_params['journal_acronym'] = match.groupdict().get('journal_acronym')
            url_params['year_vol_issue'] = match.groupdict().get('year_vol_issue')
            url_params['file'] = match.group.groupdict().get('file')
    ...
    def _get_url_params_from_url_path(self, url_params, url_path):
        for media_format, p in [
            (MEDIA_FORMAT_HTML, REGEX_OPAC_ALPHA_ARTICLE_ACRONYM_YEAR_VOL_ISSUE_LANGUAGE),
            (MEDIA_FORMAT_PDF, REGEX_OPAC_ALPHA_PDF_ACRONYM_YEAR_VOL_ISSUE_LANGUAGE),
